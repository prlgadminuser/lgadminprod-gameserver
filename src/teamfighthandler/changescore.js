...
const { UpdatePlayerPlace, UpdatePlayerWins } = require("../Database/ChangePlayerStats");
const { closeRoom } = require("../RoomHandler/closeRoom");
...
function declareWinner(room, winningTeam) {
    ...
    winningTeam.players.forEach(player => {
        ...
        if (playerObj) {
            ...
            UpdatePlayerWins(playerObj, 1); // Increase player wins
           UpdatePlayerPlace (playerObj, 1, room); // Increase player place
        }
    });
    ...
    if (secondPlaceTeam) {
        ...
        secondPlaceTeam.players.forEach(player => {
            ...
            if (playerObj) {
               ...
               UpdatePlayerPlace (playerObj, 2, room); // Increase player place
            }
        });
