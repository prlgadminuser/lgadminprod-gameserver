...
function generateRandomTarget(mapWidth, mapHeight, previousTargetX = 0, previousTargetY = 0) {
  const maxDistance = Math.min(mapWidth, mapHeight) / 2;

  // Generate random offsets within the bounds of the previous target's area
  ...
  // Adjust the new target's position relative to the previous target's position
...
function UseZone(room) {
  ...
  const mapWidth = room.mapWidth * 1;
  const mapHeight = room.mapHeight * 1;
  ...
  room.zonephases = [
    ...
    },
    {
      ...
      damagePerSecond: 4,
      zonespeed: 5,
      ...generateRandomTarget(mapWidth, mapHeight, previousTargetX, previousTargetY),
