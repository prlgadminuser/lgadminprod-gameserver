...
// Use half extents from playerhitbox if available, otherwise fallback
const playerHalfWidth = playerHitboxWidth / 2;
const playerHalfHeight = playerHitboxHeight / 2;

function isCollisionWithWalls(grid, x, y) {
  // Use playerhitbox half extents instead of magic numbers
  const xMin = x - playerHalfWidth;
  const xMax = x + playerHalfWidth;
  const yMin = y - playerHalfHeight;
  const yMax = y + playerHalfHeight;
...
function isCollisionWithPlayer(bullet, player, bulletHeight, bulletWidth, bulletAngle) {
  ...
  // Player rectangle corners using half extents
  const playerCorners = [
...
function adjustBulletDirection(bullet, wall) {
  // Use consistent wall boundaries (center ± half block size)
  const wallLeft = wall.x - halfBlockSize;
  const wallRight = wall.x + halfBlockSize;
  const wallTop = wall.y - halfBlockSize;
  const wallBottom = wall.y + halfBlockSize;
  ...
  if (minDist === distLeft || minDist === distRight) {
    ...
    bullet.direction = (180 - bullet.direction + 360) % 360;
  } else {
    // Reflect vertically
